...
var warn = new MessageButton(),
    del = new MessageButton(),
    adminmenu = new MessageActionRow();
...
client.on("messageCreate", async (message) => { // main message data
    ...
    if (!message.guild && message.author.id !== client.user.id) {
        ...
        if (!serverUser.roles.cache.has(syndicate.id)) {
            ...
            var content = `Anon ${sign} | ${message.content}`;
            embed.setAuthor({
                name: 'DarkWeb',
        ...
        if (!serverUser.roles.cache.has(syndicate.id)) {
            if (message.content) {
                embed.setDescription(`${config.emoji} ${content}`);
            };
        } else {
            if (message.content) {
        ...
        collector.on("collect", async (button) => {
            if (button.customId === "send_post") {
                if (!serverUser.roles.cache.has(syndicate.id)) {
                    var post = await dwchannel.send({ embeds: [embed] });
                } else {
                    var post = await godfather.send({ embeds: [embed] })
                }
                embed.addFields({
                    name: 'Posted By',
                ...
                return log.send({ embeds: [embed], components: [adminmenu] }) &&
                    message.author.send({ content: "Sent Your Post Successfully!" }) &&
...
client.on("interactionCreate", async interaction => {
    ...
    try {
        ...
        try { // if output is ia user
            ...
            warn.setDisabled(true);
            // client.warn.setDisabled(true);
            return output.send({
                embeds: [
                    new MessageEmbed({
                ...
                button.reply({ content: `Warning sent to ${output} whose darkweb tag is ${dtag}`, ephemeral: true }) &&
                logChannel.send({
                    embeds: [
                        new MessageEmbed({
